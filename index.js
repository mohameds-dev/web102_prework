...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; ++i){
        ...
        document.getElementById("games-container").appendChild(game_div)
    }
}
...
const totalContributions = GAMES_JSON.reduce((acc, entry) => {
    return acc + entry["backers"]
}, 0);
...
const totalRaised = GAMES_JSON.reduce((acc, entry) => {
    return acc + entry["pledged"]
}, 0);
...
function filterUnfundedOnly() {
    ...
    const underfundedGames = GAMES_JSON.filter((game) => game["goal"] > game["pledged"])
...
const unfundedGames = GAMES_JSON.filter(entry => entry["goal"] > entry["pledged"])
...
var newElement = document.createElement('p');
newElement.innerHTML = secondGame.name;
secondGameContainer.appendChild(newElement);
